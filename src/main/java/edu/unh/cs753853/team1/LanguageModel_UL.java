...
class UnigramLanguageModel {
	...
	PriorityQueue<DocumentResults> docQueue = new PriorityQueue<DocumentResults>(
			new Comparator<DocumentResults>() {
				@Override
	...
	private static ArrayList<String> analyzeByUnigram(String inputStr)
			throws IOException {
	 ...
	 */
	UnigramLanguageModel(ArrayList<Data.Page> pagelist, int numResults)
			throws IOException {
		runfilestrings = new ArrayList<>();
		results = new HashMap<>();
		maxResults = numResults;
		qp = new QueryParser("parabody", new StandardAnalyzer());
		is = new IndexSearcher(DirectoryReader.open((FSDirectory.open(new File(
				QueryParagraphs.INDEX_DIRECTORY).toPath()))));
		...
		for (Map.Entry<String, HashMap<String, Float>> queryResult : results
				.entrySet()) {
			...
			while ((docResult = docQueue.poll()) != null) {
				runfilestrings.add(queryId + "  Q0 " + docResult.paraId + " "
						+ count + " " + docResult.score + " team1-UL-L");
				count++;
				if (count >= 100)
					break;
			}
